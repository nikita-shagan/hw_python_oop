...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        minutes = self.duration * self.HOURS_IN_MINUTES
...
class SportsWalking(Training):
    ...
    K_WEIGHT_1 = 0.035
    K_WEIGHT_2 = 0.029
    METER_PER_SECOND_COEFFICIENT = 0.278
    CM_IN_M = 100
    ...
    def get_spent_calories(self) -> float:
        mean_speed = self.get_mean_speed() * self.METER_PER_SECOND_COEFFICIENT
        height_in_meters = self.height / self.CM_IN_M
        duration_in_minutes = self.duration * self.HOURS_IN_MINUTES
        return ((self.K_WEIGHT_1 * self.weight
                 + (mean_speed**2 / height_in_meters)
                 * self.K_WEIGHT_2 * self.weight) * duration_in_minutes)
...
def read_package(training_type: str, training_data: list) -> Training:
    ...
    if training_type in trainings:
        return trainings[training_type](*training_data)
